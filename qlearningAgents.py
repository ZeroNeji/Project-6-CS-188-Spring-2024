...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        #for ties, randomly choose either the best action or the new action
        for action in actions:
            qvalue = self.getQValue(state, action)
            if qvalue > maxvalue:
                maxvalue = qvalue
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.values[(state,action)] = currentvalue + self.alpha * (reward + self.discount*self.computeValueFromQValues(nextState) - currentvalue)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for i in features:
            qvalue += features[i] * self.getWeights()[i]
        return qvalue
